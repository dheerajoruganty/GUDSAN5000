...
- **Part-2:** Project checkpoint 
  ...
  - Make sure the Data collection and Data-cleaning tabs are seriously attempted and hosted on GU domains before you submit 
  - **It is important that you submit your URL. This makes things much easier for the grader. Failure to submit your URL will result in an email from the TA requesting the URL and a -5 point deduction from HW-2**
...
**In HW-2.2 you will gather data and complete the initial cleaning of your data.**
...
* Try to get as many formats as possible;
  * e.g. labeled data, qualitative data, quantitative data, text data (such as from BBC News, wikipedia, or Reddit), Geo data (such as locations), record-data etc. 
...
<!-- * **You DO NOT do exploratory visualization or analysis on the data yet.**  -->
* **NOTE: There are no limits or rules on data "size".**
  ...
  * **Data is not about size; it is about value.** Think about telling a story, think about how you might use the data you have for text analysis, clustering, classification, and regression. Get data that will enable you to perform the topics in this class and the tabs you added to your site above. ALWAYS think about solutions and not “how big or small” your data is.
...
* Have **R code that cleans record, labeled data** that has both qualitative and quantitative variables. (required). This must link to raw, cleaned-data, and the code. This should be applied to your project data NOT toy data-sets. 
  * **WHEN CODING, ALWAYS START SMALL AND SIMPLE AND THEN SCALE UP** 
*  Have **Python code that cleans text data and includes a label** (from corpus or csv or both) and you MUST use CountVectorizer. This should be applied to your project data NOT toy data-sets. 
*  (Optional but recommended) Have R code that cleans text data from a corpus for your project.
*  (Optional but recommended) Have Python code that cleans record data for your project.
...
For example, I strongly suggest that you have code for corpus --> DF, csv --> DF, json --> DF. 
...
**Provide a link to ALL CODE that you write to clean the data** so that someone could start with your raw data, run your code, & end up with your clean datasets. Make you steps EASY to see, transparent, easy to review. 
...
**One of the goals of cleaning is to retain the integrity & information contained in the original data**.
